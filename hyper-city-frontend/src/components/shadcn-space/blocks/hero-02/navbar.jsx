...
const NavLink = ({
  ...
  return (
    <li className="flex items-center group w-fit cursor-pointer">
      <div
        className={`h-0.5 bg-primary transition-all duration-300 ${item.isActive ? 'w-6 mr-4' : 'w-0 group-hover:w-6 group-hover:mr-4'}`} />
      <a
        href={item.href}
...
const Navbar = ({ navData }) => {
  ...
  return (
    <motion.header
      ...
      className={cn("z-50 w-full bg-transparent h-20 sticky top-0 flex items-center")}>
      <nav className={`max-w-7xl mx-auto sm:px-16 px-4 w-full`}>
        <div
          className={`flex items-center justify-between duration-300 ${sticky
          ...
          <div className='flex justify-between items-center gap-2 w-full'>
            ...
            <div className='flex items-center gap-2 sm:gap-6'>
              ...
              <Separator
                ...
                className={`h-5 my-auto sm:block hidden ${sticky ? "bg-white/50 dark:bg-white/50" : "bg-white/50 dark:bg-white/50"}`} />
              <div>
                <Button
                  size={"lg"}
      ...
      <Sheet open={navbarOpen} onOpenChange={setNavbarOpen}>
        <SheetContent
          ...
          <SheetHeader>
            ...
            <SheetClose
              className='absolute top-4 right-4 rounded-full dark:bg-white bg-black dark:text-black text-white p-2 cursor-pointer'>
              <X size={24} />
            </SheetClose>
          </SheetHeader>
